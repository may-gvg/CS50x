...
def analysis():
    ...
    z = pd.DataFrame(z)
    y = df2.mode().head(3)
    return render_template("analysis.html", data7=x.to_html().replace("dataframe", "dataTable-table"),
                           data8=y.to_html().replace("dataframe", "dataTable-table"),
                           data9=z.to_html().replace("dataframe", "dataTable-table"))
